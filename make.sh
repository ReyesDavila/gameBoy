...
EMULADOR=$(which mgba-sdl || which mgba-qt || which mgba || which sameboy || which visualboyadvance-m || which bgb || which emulicious)
...
while true; do
    ...
    # Buscamos archivos .c, .s y .o ignorando 'build', 'ext', 'versiones_locales' y 'test'
    SOURCES=$(find . \( -name "*.c" -o -name "*.s" -o -name "*.o" \) -not -path "./build/*" -not -path "./ext/*" -not -path "./versiones_locales/*" -not -path "./test/*")

    # Buscamos directorios de cabeceras (.h) ignorando 'build', 'ext' y 'test'
    INC_DIRS=$(find . -name "*.h" -not -path "./build/*" -not -path "./ext/*" -not -path "./test/*" -exec dirname {} + | sort -u | sed 's/^/-I/')
...
if [ $? -eq 0 ]; then
        ...
        # Si NO es la primera vez (PRIMERA_VEZ ya no está vacío), pide ENTER
        if [ -n "$YA_EJECUTADO" ]; then
            echo -e "\033[1;33m [>] Presiona ENTER para abrir el Emulador...\033[0m"
            read
        fi

        if [ -n "$EMULADOR" ]; then
            pkill -9 $(basename "$EMULADOR") 2>/dev/null
            #$EMULADOR "$ROM_FILE" > /dev/null 2>&1
            $EMULADOR -3 "$ROM_FILE" > /dev/null 2>&1
        else
            xdg-open "$ROM_FILE" > /dev/null 2>&1
        fi

        # Marcamos que ya se ejecutó la primera vez
        YA_EJECUTADO=true

        echo ""
        echo -e "\033[1;36m [!] Emulador cerrado. Presiona ENTER para volver a compilar...\033[0m"
        read
