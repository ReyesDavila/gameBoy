...
# --- [PASO 2]: CONTROL DE VERSIONES LOCALES (AUTOMÁTICO) ---
...
verificar_acceso() {
    if [ ! -f "$PASS_FILE" ]; then
        echo -e "${AMARILLO}[SISTEMA]: Configuración inicial de seguridad...${NC}"
...
# --- GESTIÓN DE PERFILES (TU ESTRUCTURA ORIGINAL) ---
gestionar_sistema() {
...
# Ejecución de la lógica de perfiles
verificar_acceso
gestionar_sistema
...
# --- [PASO 3]: ENVÍO A GITHUB (CORREGIDO) ---
...
# El secreto: Usamos 'push' con una validación inteligente
# Si el push normal falla, avisamos al usuario para que decida.
if git push -u origin main; then
    echo -e "\n${VERDE}¡ÉXITO! Tus archivos locales están en GitHub y el historial se mantuvo.${NC}"
else
    echo -e "\n${AMARILLO}Diferencia de historial detectada.${NC}"
    echo "¿Deseas forzar la subida de TUS archivos actuales? (s/n)"
    read -p "> " RESP
    if [ "$RESP" == "s" ]; then
        git push -u origin main --force
        echo -e "${VERDE}✔ GitHub actualizado con tu versión local.${NC}"
    else
        echo -e "${ROJO}Subida cancelada para proteger el historial remoto.${NC}"
    fi
fi
